"""Koordiniert ZIP-Verarbeitung, Modellaufrufe und Metrikberechnung."""
...
class BenchmarkService:
    """Verarbeitet Benchmark-Eingaben und liefert aggregierte Metriken."""
    ...
    def benchmark(
        ...
        for filepath, image_bytes in image_files.items():
            ...
            try:
                prediction_input = PredictionInput(
                    ...
                    content_type=_detect_image_content_type(filepath),
                    image_bytes=image_bytes,
                )
                prediction_result = self.prediction_port.predict(
                    prediction_input,
                    model_version=model_version,
                )
                resolved_model_version = prediction_result.model_version
                ...
                for detection in prediction_result.detections:
                    ...
                    all_predictions.append((image_id, detection.score, prediction_object))
                ...
                average_score = (
                    score_sum / len(prediction_result.detections)
                ...
                annotated_image_bytes = getattr(
                    prediction_result,
                    "annotated_image_bytes",
                    None,
                )
                if annotated_image_bytes is not None:
                    annotated_images[image_id] = annotated_image_bytes
        ...
        return calculate_benchmark_result(
            ...
            map_score=map_score,
            zip_file=_build_annotated_images_zip(annotated_images),
        )
...
def _detect_image_content_type(filepath: str) -> str:
...
def _build_annotated_images_zip(annotated_images: dict[str, bytes]) -> str:
    buffer = io.BytesIO()
    with zipfile.ZipFile(buffer, "w", compression=zipfile.ZIP_DEFLATED) as zip_file:
        for image_id, image_bytes in annotated_images.items():
            zip_file.writestr(f"{image_id}.jpg", image_bytes)
...
def _compute_map_score(
    ...
    total_ground_truth_objects = sum(len(objects) for objects in ground_truth_by_image.values())
    ...
    for image_id, _score, prediction_object in sorted_predictions:
        ...
        if best_iou >= _IOU_THRESHOLD and best_ground_truth_index >= 0:
            ...
            matched_ground_truth.setdefault(image_id, set()).add(best_ground_truth_index)
        else:
    ...
    for true_positive, false_positive in zip(
        ...
        recalls.append(cumulative_true_positives / total_ground_truth_objects)
